...
void generateShortestPath(string source, string target, imdb& db){
  ...
  while(partialPaths.front().getLength() <= 5 && !partialPaths.empty()){
    path frontPath = partialPaths.front();
    partialPaths.pop_front();
    vector<film> movies;
    db.getCredits(frontPath.getLastPlayer(), movies);
    for(int i = 0; i < movies.size(); i++){
      int size = previouslySeenFilms.size();
      previouslySeenFilms.insert(movies[i]);
      if(size == previouslySeenFilms.size()) continue;
      vector<string> cast;
      db.getCast(movies[i], cast);
      for(int j = 0; j < cast.size(); j++){
        size = previouslySeenActors.size();
        previouslySeenActors.insert(cast[j]);
        if(size == previouslySeenActors.size()) continue;
        path clone = frontPath;
        clone.addConnection(movies[i], cast[j]);
        if(cast[j] == target){
          cout << clone << endl;
          return;
        }
