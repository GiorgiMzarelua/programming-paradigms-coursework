...
void generateSentences(map<string, Definition>& grammar, string start){
  
  Production product = grammar[start].getRandomProduction();
  for(Production :: iterator cur = product.begin(); cur != product.end(); ++cur){
    string s = *cur;
    if(s[0] != '<'){
      cout << s << " ";
    }
    else generateSentences(grammar, s);
  }
  return;
}  
...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < 3; i++){
    ...
    generateSentences(grammar, "<start>");
    cout << endl;
  }
