...
(define (partition cmp pivot num-list)
  (if (null? num-list) '(() ())
     (let ((split-of-rest (partition cmp pivot (cdr num-list))))
...
(define (quicksort cmp num-list)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition cmp (car num-list) (cdr num-list))))
	(append (quicksort cmp (car split)) 
		(list (car num-list)) 
		(quicksort cmp (cadr split))))))
...
(
  define(intersection-points circlesList)
  (
    if (or (= (length circlesList) 1) (= (length circlesList) 0))
    '()
    (
      append (intersect (car circlesList) (car (cdr circlesList))) (intersection-points (cons (car circlesList) (cdr (cdr circlesList))))
      (intersection-points (cons (car (cdr circlesList)) (cdr (cdr circlesList))))
    )
  )
)
...
(
  define(distance-product p pLst)
  (
    apply * (map(lambda (x) (if(equal? x p) 1 (dist p x))) pLst)
  )
)


(
  define(rate-points pLst)
  (
    map (lambda(p) (list (distance-product p pLst) p)) pLst
  )
)


(
  define (sort-points rpLst)
  (
    quicksort (lambda (x y) (< (car x) (car y))) rpLst
  )
)
...
(
  ...
  (
    map cadr (prefix-of-list (sort-points(rate-points lst)) (quotient (length lst) 2)) 
  )
)      
...
(
  define(average-point lst)(
    list (distance-product (list (/ (sumX lst) (length lst)) (/ (sumY lst) (length lst))) lst) (list (/ (sumX lst) (length lst)) (/ (sumY lst) (length lst)))
  )
)
...
(
  define(where-am-i distances starLocs)(
    prefix-of-list (sort-points (map best-estimate (all-guesses distances starLocs))) 6
  )
)
